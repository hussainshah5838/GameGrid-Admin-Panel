...
function sheetWithAutoWidth(json) {
  ...
  ws["!cols"] = keys.map((k) => {
    ...
    const maxCellLen = json.reduce((m, r) => Math.max(m, String(r[k] ?? "").length), 0);
...
function downloadDashboardXLSX() {
  ...
  const trendsRows = trendsData.map(d => ({ Trend: d.name, Popularity: d.popularity }));
  utils.book_append_sheet(wb, sheetWithAutoWidth(trendsRows), "Trend Popularity");

  const revenueRows = revenueData.map(d => ({ Month: d.month, Revenue: d.revenue }));
  utils.book_append_sheet(wb, sheetWithAutoWidth(revenueRows), "Monthly Revenue");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#0C0F14] rounded-2xl ring-1 ring-white/10 p-4 shadow-md hover:ring-white/15 transition">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={trendsData} margin={{ top: 6, right: 12, left: 0, bottom: 0 }}>
              <defs>
              ...
              <XAxis dataKey="name" tick={{ fill: "#9CA3AF", fontSize: 12 }} axisLine={false} tickLine={false} tickMargin={8} />
              <YAxis tick={{ fill: "#9CA3AF", fontSize: 12 }} axisLine={false} tickLine={false} width={40} tickMargin={8} />
              <Tooltip
                contentStyle={{ backgroundColor: "#0C0F14", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, color: "#fff" }}
                labelStyle={{ color: "#CBD5E1" }}
              />
              <Line type="monotone" dataKey="popularity" stroke="#D0EA59" strokeWidth={2.5}
                    dot={{ r: 3, stroke: "#0C0F14", strokeWidth: 2 }} activeDot={{ r: 5 }} />
              <Line type="monotone" dataKey="popularity" stroke="none" fill="url(#trendGlow)" fillOpacity={1} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-[#0C0F14] rounded-2xl ring-1 ring-white/10 p-4 shadow-md hover:ring-white/15 transition">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={revenueData} margin={{ top: 6, right: 12, left: 0, bottom: 0 }}>
              <defs>
              ...
              <CartesianGrid stroke="rgba(255,255,255,0.06)" vertical={false} />
              <XAxis dataKey="month" tick={{ fill: "#9CA3AF", fontSize: 12 }} axisLine={false} tickLine={false} tickMargin={8} />
              <YAxis tick={{ fill: "#9CA3AF", fontSize: 12 }} axisLine={false} tickLine={false} width={48} tickMargin={8} />
              <Tooltip
                contentStyle={{ backgroundColor: "#0C0F14", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, color: "#fff" }}
                labelStyle={{ color: "#CBD5E1" }}
              />
              <Bar dataKey="revenue" fill="url(#revBars)" radius={[8, 8, 4, 4]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
